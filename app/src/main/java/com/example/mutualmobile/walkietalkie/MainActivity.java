...
public class MainActivity extends AppCompatActivity {
  ...
  private void initialSetup() {
    ...
    formGroup = (Button) findViewById(R.id.formgroup);
    // add onClick Listeners
    centerDeviceIcon.setOnClickListener(new View.OnClickListener() {
  ...
  private void searchDevice(View v) {
    int view_id = v.getId();
      ...
      if (peersList.getDeviceList().size() != 0) {
        ...
        for (CustomDevice d : device_already_present) {
          if (rippleBackground != null) {
            rippleBackground.removeView(d.icon_view);
